...
// 예약 취소 API
app.post('/api/admin-cancel-reservation', async (req, res) => {
  const { reservation_id, reason } = req.body;

  try {
    const reservationSelectSql = 'SELECT * FROM reservations WHERE id = ?';
    db.query(reservationSelectSql, [reservation_id], async (err, reservationResults) => {
      if (err) {
        console.error('Database query error:', err);
        return res.status(500).send('Database query error');
      }
